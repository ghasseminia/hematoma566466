...
def train_model():
	...
	for index in range(1,26):
		...
		#X_max_slice = max(enumerate(X_hematoma_slices), key=lambda x: x[1][1])
		#y_max_slice = max(enumerate(y_hematoma_slices), key=lambda x: x[1][1])

		#index_offset = y_max_slice[1][0] - X_max_slice[1][0]
		...
		for i in range(len(X_hematoma_slices)):
			...
			#y_slice = img1_data_arr[:,:,X_hematoma_slices[i][0]]
			#if np.any(np.logical_and(X_slice,y_slice) == 1):
			X.append(X_slice)
			y.append(y_slice)
			print(np.count_nonzero(X_slice[:,:]),np.count_nonzero(y_slice[:,:]))
