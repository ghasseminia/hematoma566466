...
if __name__ == '__main__':
	...
	weights_filename = './train images/weights.h5'
	architecture_filename = './train images/model.json'
	...
	for index in range(imgrange_start,imgrange_end):
		...
		img_slices.append([(img0.shape[0],img0.shape[1],6), img0.header, img0.affine])
	...
	for index in range(imgrange_start-1,imgrange_end-1):
		...
		for x in range(0, img_slices[index][0][0]):
			for y in range(0, img_slices[index][0][1]):
				for z in range(temp_slice, temp_slice + 6):
		...
		nib.save(img_pred, os.path.join('./',str(index+1) + '_pred_image.nii'))
		temp_slice = temp_slice + 6
