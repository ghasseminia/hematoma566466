...
if __name__ == '__main__':
    ...
    for index in range(0,imgrange_end - imgrange_start):
        ...
        img0_data_arr = np.asarray(img0_data)
        ##################################3
        scan_at_t1 = os.path.join(str(index+imgrange_start) + '_24h_ICH-Measurement' + '.nii')
        img1 = nib.load(scan_at_t1)
        img1_data = img1.get_data()
        img1_data_arr = np.asarray(img1_data)        
...
########
        ...
        dice = dice_score(img_pred_arr,img1_data_arr)
        print('dice score is')
        print(dice)
        ############
        error = binary_error(img_pred_arr,img1_data_arr)
        print('binary error is')
        print(error)
        ##########
